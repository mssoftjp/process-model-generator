...
- **S-33** Route a return edge around the perimeter on the side opposite its target, not through the loop interior. A same-row loop prefers the lower side. A cross-row or cross-lane loop considers the source-side perimeter and enters or exits directly through an available north or south port. Fall back to the legacy channel above the target when a perimeter candidate is illegal or worsens the whole-diagram score. In the baseline route, a reverse channel exits vertically north when S-56 leaves the top port available and the source column is clear up to the target channel under S-58; a gateway source keeps the right gutter in the baseline, and only the improved candidate may take it north (the baseline form regressed keihi in cycle 1). Non-sequence returns keep the S-57 offset band and the right gutter.
...
Apply in this order: direct, row-column, rise, drop, row approach, channel approach. The final pattern is always the legal fallback. The one-bend shapes come in mirror pairs: `drop` and `rise` travel the column first, `row-column` travels the row first.
...
- **S-45 rise**: The mirror of S-41 for an upward non-main gateway branch, used by the improved candidate: leave north, travel the own column center to the target row, turn, and enter from the left, when the own column and the target row are clear and the S-36 and S-58 reservations succeed.
- **S-44 row-column**: For a non-gateway source, or a gateway source on the main path (which leaves east under S-50), whose target is a gateway, event, or (for data associations) task in a later column and a different row, leave east, travel the source-row baseline to the target column, then travel the target column center and enter the target's south vertex from below or north vertex from above. Conditions are static: the source-row cells up to and including the target column are clear, the target column is clear between the two rows, the S-36 and S-58 reservations succeed, and the entered vertex is not used as an exit (the same predicates as S-56). Several edges converging on one target share the vertical (S-32, S-91), which draws a join bus. A gateway source uses this pattern only for its main-path edge, which leaves east under S-50; its other branches keep the vertex grammar (S-41, S-45). This is the natural shape of a cross-lane join entry and of a document feeding a task; it replaces the hook that S-43 produced by climbing to the channel above the target and coming back down.
...
- **S-51** Entries normally use the west side. Cross-row gateway entries and return edges normally use north. South entry is allowed when static analysis proves the port is unused, removing an unnecessary hook around the top. A gateway whose only incoming sequence flow arrives from another row, with no same-row predecessor, keeps its west vertex free, so that single entry uses the ordinary left-entry patterns (S-42, S-43) instead of the north vertex.
...
- **S-55** Data associations (C-63) enter non-document nodes through a vertex because the west port belongs to incoming sequence flows. A same-row straight data association uses a connection point 10 px above the baseline and never shares the sequence-flow boundary point. It exits east, or south for a target directly below. Prefer a straight bottom-to-top drop in document-flow patterns; the mirror image, a document directly below the task that reads it, is a straight top-to-bottom rise when the task's south face has no other non-sequence traffic and no other writer enters the document's north face. A document on the same row is entered from the west because documents have no sequence flows; from another row a data association may enter a document through its north or south vertex under S-44.
...
- **S-57** Message flows (C-60) use distinct exits for distinct flow kinds. Between adjacent pools, the first candidate is the Z shape: leave through the vertex facing the other participant (south when the receiver is below, north when above), travel the sender's column center to the inter-pool corridor, cross the corridor horizontally, and travel the receiver's column center into its facing vertex; when both endpoints share a column the message is a single vertical segment. Column centers are reserved under S-58. Two verticals reaching the corridor band at the same column from opposite sides are allowed only when the S-37 nesting order places the upper-side run above the lower-side run without contradiction; an X-shaped pair, where each run's endpoint lies inside the other's interval, is refused because the band would make them overlap. A request and its reply, or a message arriving where another leaves, may share a task's column and face: the S-54 entry slots spread them into parallel lines, so the column registry treats runs ending on the same task face as shareable, except two-point straight runs, which are never slotted and require both faces to carry no other non-sequence traffic. The Z is used only when the facing vertices are statically free (an event label moved by S-62 frees its south side; a gateway with other messages, a gateway with a downward branch, and a node whose column is blocked fall through). Two messages that use the same face of one task (a request and its reply, or a message arriving where another leaves) may share the column reservation: the entry slots of S-17 spread them along the face, so they run as parallel lines. A same-column single vertical has no slots and never shares. Otherwise they enter and leave through dedicated points on the gutter-facing vertical edges and travel gutter → inter-pool corridor → gutter. Do not create a meaningless small bend immediately outside a node or place a long horizontal component inside a participant. Offset a sender by ±8 px toward the communication direction and a receiver by ±12 or ±14 px in the opposite direction appropriate to its attachment side, so request and reply ports remain distinct. Treat these offsets as desired centerline positions and clip the final endpoint to the actual circle or diamond boundary. Move an event label above the event when a message connects to its lower half. Other messages use the standard target-row channel rule. Right-exit fallbacks for non-sequence flows use `cy + 10` (`+8` for messages) regardless of sequence exits, and never place a gutter-ending horizontal segment on the baseline (S-36). Reserve `cy - 10` for a same-row straight association under S-55.
- **S-58** Reserve vertical runs through column centers, including drops, straight descents, and vertical message exits, with `reserveColRun`. A shared source may share a trunk and a shared target may share a convergence; other overlaps are forbidden, except the task-face sharing of S-57. A two-point straight run is marked exclusive because no slot separation can apply to it. Because the mutual protection from S-36 does not cover verticals ending in a channel, the cell immediately beyond the terminal channel must also be clear.
