...
- `W-436` identifies a route crossing an external node label, with edge ID, node ID, segment and coordinates. It is separate from edge-label collisions and syntax/graph validation. `N-437` records adopted connected-input/output routing candidates; `N-438` records a local placement alternative adopted after complete rerouting and a reduction in geometric intersections. Library comparisons can pass `optimizePlacement: false` to fix the initial placement while retaining routing and text checks; this is a rendering option and never changes business topology.
